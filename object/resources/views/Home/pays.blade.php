...
<script type="text/javascript" async="" src="{{ url('Home/pay/mstr.js')}}"></script>
<script type="text/javascript" async="" src="{{ url('Home/pay/unjcV2.js') }}"></script>
<script type="text/javascript" async="" src="{{ url('Home/pay/mstr(1).js') }}"></script>
<!-- <script type="text/javascript" async="" src="{{ url('Home/pay/jquery.statData.min.js') }}"></script> -->
<script type="text/javascript" async="" src="{{ url('Home/pay/xmst.js') }}"></script>
        ...
        <div class="topbar-info" id="J_userInfo">
            <span class="user">
                <a rel="nofollow" class="user-name" href="{{ url('home/userinfo')}}" target="_blank">
                    <span class="name">{{ session('user')->username }}</span><i class="iconfont"></i>
                </a>
                ...
                <div class="order-info clearfix">
                    <div class="fl">
                        ...
                        <p class="post-info" id="J_postInfo">
                            {{ $address->consignee }} {{ $address->consignee_phone }}
                            &nbsp;&nbsp;{{ $address->province }}&nbsp;&nbsp;{{ $address->city }}&nbsp;&nbsp;{{ $address->county }}&nbsp;&nbsp;{{ $address->detail }}</p>
                    </div>
                    <div class="fr">
                        <p class="total">
                            应付总额：<span class="money">
                            @foreach($order as $b)
                            <input type='hidden' class='f' name='total' value="{{ $b->total }}">
                            @endforeach
                            <em id='total'></em>
                            元</span>
                        </p>
                ...
                <div class="payment-box ">
                    ...
                    <div class="payment-body">
                        <ul class="clearfix payment-list J_paymentList J_linksign-customize">
                            <li class="J_bank">
                                <img id="J_weixin" src="{{ url('Home/pay/wechat0715.jpg') }}" alt="微信支付" style="margin-left: 0;">
                            </li>
                        
                            <li class="J_bank">
                                <input type="radio" name="payOnlineBank" id="alipay" value="alipay">
                                <img id='zfb' src="{{ url('Home/pay/alipay-0718-1.png') }}" alt="支付宝" style="margin-left: 0;">
                            </li>             
                        </ul>
                        <div class="event-desc">
                            <p>微信支付：关注小米手机微信公众号，支付成功后可领取3-10元电影票红包。</p><p>支 付 宝：支付宝扫码支付满38元，参与赢取1999元红包</p><a href="" class="more" target="_blank">了解更多&gt;</a>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="modal modal-hide fade modal-weixin-pay in" id="J_modalWeixinPay" aria-hidden="false" style="display: none;">
    <div class="modal-hd">
        <span class="title">微信支付</span>
        <a class="close" id='close' href="javascript: void(0);" ><i class="iconfont"></i></a>
    </div>
    <div class="modal-bd" id="J_showWeixinPayExample">
        <div class="code" id="J_weixinPayCode"><a href="javascript:weixin()"><img   src="{{ url('Home/pay/weixin.png') }}"></a></div>
        <div class="msg">
...
<div class="modal modal-hide fade modal-weixin-pay in" id="J_modalzfbPay" aria-hidden="false" style="display: none;">
    <div class="modal-hd">
        <span class="title">支付宝支付</span>
        <a class="close" id='close2' href="javascript: void(0);" ><i class="iconfont"></i></a>
    </div>
    <div class="modal-bd" id="J_showWeixinPayExample">
        <div class="code" id="J_weixinPayCode"><a href="javascript:zfb()"><img   src="{{ url('Home/pay/weixin.png') }}"></a></div>
        <div class="msg">
            请使用 <span>支付宝</span> 扫一扫<br>二维码完成支付
        </div>
    </div>
   
    <div class="example" id="J_weixinPayExample" style="display: none; opacity: 1;"></div>
</div>
<form action="{{ url('home/pays/0') }}" method='post' name='weixin'>
    {{ csrf_field() }}
    {{ method_field('PUT') }}
    <input type='hidden' name='gidd'>
    <input type='hidden' name='payname' value='微信支付'>
</form>    
<form action="{{ url('home/pays/0') }}" method='post' name='zfb'>
    {{ csrf_field() }}
    {{ method_field('PUT') }}
    <input type='hidden' name='gidd'>
    <input type='hidden' name='payname' value='支付宝支付'>
</form>  
<script type="text/javascript">
    function weixin()
    {
        ...
        // alert(id);
        $('input[name*="gidd"]').val(id);
        var form = document.weixin;
        form.submit();
    }
    ...
    function zfb()
    {
        ...
        // alert(id);
        $('input[name*="gidd"]').val(id);
        var form = document.zfb;
        form.submit();
    }
    $('#J_weixin').click(function(){
        $('.modal-backdrop').css('display','block');
        $('#J_modalWeixinPay').css('display','block');
    });
    $('#zfb').click(function(){
        $('.modal-backdrop').css('display','block');
        $('#J_modalzfbPay').css('display','block');
    });
    $('#close').click(function(){
        $('.modal-backdrop').css('display','none');
        $('#J_modalWeixinPay').css('display','none');
    });
    $('#close2').click(function(){
        $('.modal-backdrop').css('display','none');
        $('#J_modalzfbPay').css('display','none');
    });
</script>
